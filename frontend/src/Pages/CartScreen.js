      ...
      <div className="cart-screen">
        ...
        <div className="cart container m-auto">
          <div className="cart-list ">
            ...
            <li>
                ...
                <div className="cart-name">
                  ...
                  <div>
                Qty: <select onfocus='size=5' onblur='size=1' onchange='size=1' className="qty-select" id="${item.product}">
                ${[...Array(item.countInStock).keys()].map((x) =>
                    item.qty === x + 1
                      ? `<option selected value="${x + 1}">${x + 1}</option>` : `<option value="${x + 1}">${x + 1}</option>`)
                    }
                </select>
                <button type="button" className="delete-btn" id="${item.product}">Delete</button>
                  </div>
                </div>
          ...
          <div className="cart-action">
            ...
            <button id="checkout-btn" className="check-btn">
              Proceed to Checkout
            </button>
          </div>
        </div>
